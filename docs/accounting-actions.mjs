import {ACTION_COLUMN_HEADERS, ACTION_SKUS, ACTION_COLORS, shortActionText} from './actions-model.mjs?v=76';
...
export function addAccountingActions({grid,rows,desktop},ui,getAction){
  ...
  if(measure)measure.font=`700 ${fontSize}px Arial`;
  const width=text=>Math.ceil((measure?.measureText(text).width||text.length*fontSize*.6)+12);
  const actions=rows.map(row=>!row.isHeader&&!row.isTotal?(ACTION_SKUS[row.sku]||[]).map(getAction):[]);
  // Reserve room for all three icons without increasing the established row height.
  const columnWidth=Math.max(160,...actions.flat().filter(Boolean).map(action=>width(shortActionText(action))+70));
